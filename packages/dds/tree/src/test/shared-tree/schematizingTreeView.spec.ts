...
import { GCChecker, checkoutWithContent, createTestUndoRedoStacks, insert } from "../utils.js";
// eslint-disable-next-line import/no-internal-modules
...
describe("SchematizingSimpleTreeView", () => {
	...
	it.only("does not leak views when we dispose", () => {
		const gcChecker = new GCChecker();
		...
		gcChecker.addObject(view);
		view[disposeSymbol]();
		gcChecker.gcAndVerify();
	});
});
