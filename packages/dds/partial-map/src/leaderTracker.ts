 ...
 */
 export interface ILeaderEvents extends IEvent {
	(event: "promoted" | "demoted", listener: () => void);
}
 ...
 */
 export class LeaderTracker extends TypedEventEmitter<ILeaderEvents> {
    ...
    public constructor(private readonly runtime: IFluidDataStoreRuntime) {
        super();
        // This code is somewhat duplicated from OldestClientObserver
        // because it currently depends on the container runtime
    ...
    /**
	 * Computes the oldest client in the quorum, true by default if the container is detached and
     * false by default if the client isn't connected.
	 * TODO:#55900: Get rid of copy-pasted OldestClientObserver code
	 */
